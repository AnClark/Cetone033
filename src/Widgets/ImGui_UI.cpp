...
void ImGuiUI::onImGuiDisplay()
{
    ...
    ImGui::Begin("Toolbar", NULL, ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoTitleBar);
    {
        ...
        if (ImGui::Button("Get Preset", ImVec2(100, 20)))
        {
            PresetManager pm(this->ui);
            SynthPreset_DPF p = pm.dumpPreset(String("Current Preset"));

            d_stdout("Preset item:");
            printf("{\n.Name = \"%s\",\n", p.Name.buffer());
            printf(".Parameter = {\n");
        ...
        if (ImGui::Button("Load an example preset", ImVec2(150, 20)))
        {
            PresetManager pm(this->ui);
            pm.applyFactoryPreset(0);
