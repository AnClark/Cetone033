...
MenuWidget::MenuWidget(Widget* widget) noexcept
    ...
    , font_item_size(12.0f * getScaleFactor())
    , font_section_size(12.0f * getScaleFactor())
...
bool MenuWidget::mouseEvent(const MouseEvent& ev, const Point<int>& offset)
{
    ...
    ev_offset.pos = Point<double>(ev.pos.getX() + offset.getX() - getAbsoluteX(),
        ...
        for (size_t i = 0; i < items.size(); ++i) {
            ...
            if (!is_section && is_enabled) {
                ...
                if (item_bounds.contains(ev.pos)) {
                    ...
                    // selected_i = i;  // This should be bypassed since we don't always want a select menu. Use MenuWidget::setItemSelected() instead.
                    hide();
                    return true;
                }
            }
...
bool MenuWidget::onMotion(const MotionEvent& ev)
{
    ...
    if (menu_bounds.contains(ev.pos)) {
        ...
        for (size_t i = 0; i < items.size(); ++i) {
            ...
            if (static_cast<int>(i) != selected_i && !items[i].is_section && item_bounds.contains(ev.pos)) {
